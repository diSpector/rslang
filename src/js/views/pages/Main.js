...
const Login = {
  ...
  afterRender: async (model) => {
    if (await model.checkUser()) {
      ...
      document.querySelector('.mainPage_block-team').onclick = () => {
        window.location.replace(`${window.location.origin}/#/team`);
      };

      document.querySelector('.mainPage_block-promo').onclick = () => {
        window.location.replace(`${window.location.origin}/#/promo`);
      };
